...
DEFAULT_DATA = {
    ...
    "finished_matches": [],
    "round_stage": "",
    "item_authors": {},      # item -> user_id
    "user_items": {},        # user_id -> item
    "cup_history": []        # persistent history
}
...
def load_data():
    try:
        ...
        if r.status_code == 200:
            ...
            if not isinstance(data["item_authors"], dict):
                data["item_authors"] = {}
            if not isinstance(data["user_items"], dict):
                data["user_items"] = {}
            if not isinstance(data["cup_history"], list):
                data["cup_history"] = []
...
def user_allowed(member, allowed_roles):
...
# =========================================================
# AUTO LOCK + MATCH POSTING (UNCHANGED LOGIC)
# =========================================================
...
async def _lock_match(guild, channel, data, sha, reason, ping_everyone, reply_msg):
    ...
    try:
        text = f"{'@everyone ' if ping_everyone else ''}🔒 **Voting closed** ({reason})"
        if reply_msg:
...
async def addwcitem(interaction: discord.Interaction, items: str):
    ...
    return await interaction.response.send_message("⚠️ Nothing added.", ephemeral=True)
...
async def removewcitem(interaction: discord.Interaction, items: str):
    ...
    return await interaction.response.send_message("⚠️ Nothing removed.", ephemeral=True)
...
async def scheduled_tasks():
    ...
    _ = now  # placeholder (no changes to your match flow)

    ...
    async def confirm_end(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if winner:
            ...
            if stats:
                ...
                embed = discord.Embed(
                    title="<:crown:1468289809612275793> CHAMPION CROWNED - {winner['name'].upper()} <:crown:1468289809612275793>", 
                    description=f"<:winner:1462297763260923946> {winner['name']} <:winner:1462297763260923946> is The World Cup of {cat_name} Winner!\n\n **Submitted by:** {winner['user']}\n\n", 
                    color=0xf1c40f
                )
    ...
    async def setup_hook(self):
        ...
        if match and "item_a" in match:
            self.add_view(MatchView(
                match['item_a'], 
                match['item_b'], 
                match.get('round_name', 'Grand Final'),
                match.get('match_num', '1')
            ))
        else:
    ...
    async def resolve_match(self, data, sha, interaction=None):
        ...
        async with self.processing_lock:
            ...
            if is_tie and not is_final:
                ...
                # Added Submitter info to the reveal message too
                await tie_msg.edit(content=f"🏁 **Tie-Breaker Complete!**\nResult: **{winner['name']}** (Submitted by {winner.get('user', 'Unknown')}) wins the flip!")
            ...
            if not is_final:
                ...
                # RESTORED: The Submitter field and Custom Emoji
            ...
            if is_final:
                if interaction:
                    await interaction.followup.send(
                        "**Grand Final completed successfully.** No public embed posted yet. Use `/endcup` to trigger the final ceremony.", 
                        ephemeral=True
                    )
            else:
